...
void Ui::render_board(BitBoard &board) {
    ...
    for (size_t row = 0; row < SIZE; row++) {
        for (size_t col = 0; col < SIZE; col++) {
            ...
            if (mouse_in_cell && cell == Cell::None) {
                DrawRectangleRec(rec, MOUSE_IN_EMPTY_COLOR);
            } else {
                DrawRectangleRec(rec, EMPTY_COLOR);
            }
            switch (cell) {
                case Cell::White: render_white(rec, mouse_in_cell); break;
                case Cell::Black: render_black(rec, mouse_in_cell); break;
